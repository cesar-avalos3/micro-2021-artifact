...
for bench in benchmarks:
    ...
    for argpair in argslist:
        ...
        if "other_stats" in options.collect:
            ...
            if options.nsight_profiler:
                metrics_ = 'l1tex__t_sectors_pipe_lsu_mem_global_op_ld.sum,l1tex__t_sectors_pipe_lsu_mem_global_op_st.sum,l1tex__t_sectors_pipe_lsu_mem_local_op_ld.sum,smsp__inst_executed.sum,smsp__thread_inst_executed_per_inst_executed.ratio,smsp__sass_inst_executed_op_global_atom.sum,smsp__inst_executed_op_global_ld.sum,smsp__inst_executed_op_global_st.sum,smsp__inst_executed_op_shared_ld.sum,smsp__inst_executed_op_shared_st.sum,smsp__inst_executed_op_surface_atom.sum,smsp__inst_executed_op_surface_ld.sum,smsp__inst_executed_op_surface_red.sum,smsp__inst_executed_op_surface_st.sum,sass__inst_executed_global_loads,sass__inst_executed_global_stores,launch__grid_size,gpc__cycles_elapsed.avg '
                
                sh_contents += "\nexport CUDA_VERSION=\"" + cuda_version + "\"; export CUDA_VISIBLE_DEVICES=\"" + options.device_num +\
                    "\" ; timeout 30m nv-nsight-cu-cli --metrics " + metrics_ +\
                    " --csv --page raw --target-processes all " + kernel_number +\
                    " " + exec_path + " " + str(args) +\
                    " | tee " + os.path.join(this_run_dir,"out.csv")
